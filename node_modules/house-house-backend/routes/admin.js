...
router.delete('/laminas/:id', async (req, res) => {
    const { id } = req.params;
    try {
        if (!mongoose.Types.ObjectId.isValid(id)) {
            return res.status(400).json({ error: 'Identificador inválido' });
        }
        const eliminado = await Lamina.findByIdAndDelete(id);
        if (!eliminado) return res.status(404).json({ error: 'Lámina no encontrada' });
        res.json({ mensaje: 'Lámina eliminada' });
    } catch (err) {
        console.error('❌ ERROR EN DELETE /laminas/:id:', err.message);
